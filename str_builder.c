...
str_builder_t* str_builder_create(void)
{
    ...
    sb->str = xmalloc(str_builder_min_size);
    *sb->str = '\0';
...
static void str_builder_ensure_space(str_builder_t* sb, size_t add_len)
{
    ...
    sb->str = xrealloc(sb->str, sb->alloced);
}
...
{
    if (sb == NULL || str_isempty(str))
        return;

    str_builder_add_sub_str(sb, str, strlen(str));
}

void str_builder_add_sub_str(str_builder_t* sb, const char* str, size_t len)
{
    if (sb == NULL || str_isempty(str) || len == 0)
        return;
